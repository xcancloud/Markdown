...
- **Code Block UX** — Copy, download (language-based extension; optional `file:` / comment meta for filename), HTML sandbox preview; optional **Apply** (`onApplyCode` for coding agents)
- **Chat presets** — optional `CHAT_PROCESSOR_OPTIONS` / `createStreamingProcessor` (does not change library defaults)
...
- **Streaming** — `streaming` prop for live SSE/chunked content (short debounce coalesce, skip Shiki/Mermaid while streaming, full highlight on stream end, cursor affordance)
...
- **Dual Build** — ESM + CJS, TypeScript declarations, subpath entries (`/viewer`, `/renderer`, `/editor`)
...
>
> For viewer / renderer only (no editor chrome), prefer:
> ```tsx
...
### Subpath entries (smaller bundles)

| Entry | Use case | CodeMirror |
| --- | --- | --- |
| `@xcan-cloud/markdown` | Full surface (backward compatible) | In dependency graph |
| `@xcan-cloud/markdown/viewer` | Lightweight preview / SSR | No |
| `@xcan-cloud/markdown/renderer` | Streaming, TOC, Mermaid, code actions | No |
| `@xcan-cloud/markdown/editor` | Rich editing (includes preview) | Yes |

```tsx
// Chat / read-only — does not pull CodeMirror
...
> `CHAT_*` presets are **opt-in**; they do not change `createProcessor` / `showToc` library defaults.
...
| `debounceMs` | `number` | `150` | Render debounce; still applied while `streaming`, floored at 50ms to coalesce tokens |
...
| `components` | `Partial<Record<string, ComponentType<any>>>` | — | **Deprecated / no-op**: HTML pipeline cannot map React components; value is ignored |
| `onApplyCode` | `(info: CodeBlockInfo) => void` | — | When set, shows an Apply button on fenced code blocks (injected after streaming ends) |
| `applyLabel` | `string` | i18n `applyCode` | Apply button label |
| `streaming` | `boolean` | `false` | Live stream: disables Shiki/Mermaid fence transforms for cheap updates; full re-render when streaming ends |
...
| `mermaid` | `boolean` | `true` | Mermaid code blocks (`false` skips mermaid-container rewrite) |
...
| Export | Description |
| --- | --- |
| `createProcessor`, `renderMarkdown`, `renderMarkdownSync`, `parseToAst`, `resolveProcessorOptionsForRender`, `processorOptionsCacheKey` | Core unified pipeline and render helpers |
| `CHAT_PROCESSOR_OPTIONS`, `STREAMING_PROCESSOR_OPTIONS`, `CHAT_RENDERER_DEFAULTS`, `createChatProcessor`, `createStreamingProcessor` | Chat / streaming presets (**does not** change `createProcessor` defaults) |
...
| `MarkdownWorkerRenderer`, `RenderCache`, `sharedRenderCache`, `splitHtmlBlocks` | Worker / cache utilities |
